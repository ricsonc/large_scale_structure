...
void group_bodies(Rect rect, Bodies * bodies, int n, 
...
Tree built_qtree_r(Rect rect, Body * bodies, int n){
        Tree tree = {.node = {.rect = rect, .body = NULL}, .trees = Tree[4]};
        Rect * rects = Rect[4];
        Body ** groups = Body[4][n];
        int * ns = int[4];
        groupbodies(rect, bodies, n, rects, groups, ns);
        //recursively call
        int num_child = 4;
        for(int j = 0; j < 4; j++){
                if(ns[j] > 1){
